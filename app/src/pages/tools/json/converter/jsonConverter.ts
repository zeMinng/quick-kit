...
export const EditorInputCodeOptions: EditorOptions = {
  fontSize: 14,
  automaticLayout: true,
  minimap: { enabled: false },
  scrollBeyondLastLine: false,
  formatOnPaste: true,           // 粘贴自动格式化
  tabSize: 2,                    // 规范缩进为 2 空格
  bracketPairColorization: { enabled: true }, // 括号彩色化
  guides: {
    indentation: true,
    bracketPairs: true,          // 括号对齐线
  },
  scrollbar: { verticalScrollbarSize: 8, horizontalScrollbarSize: 8 }
}

export const EditorOutputCodeOptions: EditorOptions = {
  fontSize: 14,
  readOnly: true,
  domReadOnly: true,
  automaticLayout: true,
  ...
  minimap: { enabled: false },
  scrollBeyondLastLine: false,
  contextmenu: true,             // 允许右键复制
  lineDecorationsWidth: 10,      // 压缩左侧断点槽宽度
  glyphMargin: false,            // 彻底关闭字形槽
  links: false,                  // 禁用 URL 链接高亮（大文件提速）
