...
ElevenLabs reads like a quietly editorial print magazine that happens to be a voice-AI product. The base canvas is white `{colors.canvas}` (#ffffff) holding warm near-black ink `{colors.ink}` (#0c0a09). The brand voltage is **photographic, not chromatic**: soft pastel atmospheric gradient orbs (mint, peach, lavender, sky, rose) drift through the page as the only "color" moments. There is no neon accent, no saturated CTA color, no dark-canvas dev-tools atmosphere.
...
**Key Characteristics:**
- White canvas, warm near-black ink. No saturated CTA color.
...
### Surface
- **Canvas** (`{colors.canvas}` — #ffffff): White page floor.
- **Canvas Soft** (`{colors.canvas-soft}` — #fcfcfc): Lighter band for subtle alternating sections.
- **Canvas Deep** (`{colors.canvas-deep}` — #0c0a09): Same as ink — used for the rare dark-mode hero (Agents page).
- **Surface Card** (`{colors.surface-card}` — #fafafa): Card and panel fills — warm neutral lift from the white canvas.
- **Surface Strong** (`{colors.surface-strong}` — #f3f3f3): Badges, voice-icon plates.
...
### Hairlines
- **Hairline** (`{colors.hairline}` — #ececec): Default 1px divider.
- **Hairline Soft** (`{colors.hairline-soft}` — #f5f5f5): Lighter divider.
- **Hairline Strong** (`{colors.hairline-strong}` — #dddddd): Stronger panel outline.
...
The system uses **hairline + soft drop**. Cards float above the white canvas via 1px hairlines and a single subtle shadow tier. Atmospheric depth comes from gradient orbs.

| Level | Treatment | Use |
|---|---|---|
| Flat (canvas) | `{colors.canvas}` (#ffffff) | Body bands, footer |
| Card | `{colors.surface-card}` (#fafafa) | Content cards |
